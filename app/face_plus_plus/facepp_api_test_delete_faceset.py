...
face_two = '/mnt/hgfs/shared/test.jpg'
# 本地图片的地址,调用demo前请填上内容
# Local picture location, please fill in the contents before calling demo
face_search = '/mnt/hgfs/shared/test.jpg'
...
def print_result(hit, result):
    def encode(obj):
        ...
        if type(obj) is dict:
            return {encode(v): encode(k) for (v, k) in obj.items()}
        if type(obj) is list:
...
# 首先，导入SDK中的API类
from app.face_plus_plus.facepp import API, File
